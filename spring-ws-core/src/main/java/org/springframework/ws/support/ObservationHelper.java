...
public class ObservationHelper {
	...
	public ObservationHelper() {
		...
		try {
			...
			this.logger.warn(
					"Could not create SAX parser, observation keys for Root element can be reported as 'unknown'.", ex);
		}
		this.saxParser = parser;
	}

	/**
	 * Try to find the root element QName for the given source. If it isn't possible to
	 * extract the QName, a QName with the values 'unknown:unknown' is returned.
	 */
	public QName getRootElement(Source source) {
		...
		if (source instanceof StreamSource) {
			...
			try {
				this.saxParser.parse(getInputSource((StreamSource) source), handler);
				return handler.getRootElementName();
			}
			catch (SAXException | IOException ex) {
				WARN_THEN_DEBUG_LOGGER.log("Exception while handling request, reporting Root element as 'unknown'", ex);
				return UNKNOWN_Q_NAME;
			}
		}
		if (source instanceof SAXSource) {
			...
			RootElementSAXHandler handler = new RootElementSAXHandler();
			try {
				this.saxParser.parse(getInputSource((SAXSource) source), handler);
				return handler.getRootElementName();
			}
			catch (SAXException | IOException ex) {
				WARN_THEN_DEBUG_LOGGER.log("Exception while handling request, reporting Root element as 'unknown'", ex);
	...
	static class RootElementSAXHandler extends DefaultHandler {
		...
		public void startElement(String uri, String localName, String qName, Attributes attributes) {
			if (this.rootElementName == null) {
				this.rootElementName = new QName(uri, localName);
			}
		}
