...
public class DefaultWebServiceEndpointConvention implements WebServiceEndpointConvention {
	...
	private static final KeyValue EXCEPTION_NONE = KeyValue
		.of(EndpointObservationDocumentation.LowCardinalityKeyNames.EXCEPTION, KeyValue.NONE_VALUE);
	...
	public KeyValues getLowCardinalityKeyValues(WebServiceEndpointContext context) {
		return KeyValues.of(exception(context), localPart(context), namespace(context), outcome(context), path(context),
				soapAction(context));
	}
	...
	private KeyValue localPart(WebServiceEndpointContext context) {
		return EndpointObservationDocumentation.LowCardinalityKeyNames.LOCALPART.withValue(context.getLocalPart());
	}

	private KeyValue namespace(WebServiceEndpointContext context) {
		return EndpointObservationDocumentation.LowCardinalityKeyNames.NAMESPACE.withValue(context.getNamespace());
	}

	private KeyValue outcome(WebServiceEndpointContext context) {
		return EndpointObservationDocumentation.LowCardinalityKeyNames.OUTCOME.withValue(context.getOutcome());
	}

	private KeyValue soapAction(WebServiceEndpointContext context) {
		return EndpointObservationDocumentation.LowCardinalityKeyNames.SOAPACTION.withValue(context.getSoapAction());
	}

	private KeyValue exception(WebServiceEndpointContext context) {
		if (context.getError() != null) {
			return EndpointObservationDocumentation.LowCardinalityKeyNames.EXCEPTION
	...
	private KeyValue path(WebServiceEndpointContext context) {
		return EndpointObservationDocumentation.LowCardinalityKeyNames.PATH.withValue(context.getPath());
	}

	private KeyValue pathInfo(WebServiceEndpointContext context) {
		if (context.getPathInfo() != null) {
			return EndpointObservationDocumentation.HighCardinalityKeyNames.PATH_INFO.withValue(context.getPathInfo());
		}
		return null;
	}
