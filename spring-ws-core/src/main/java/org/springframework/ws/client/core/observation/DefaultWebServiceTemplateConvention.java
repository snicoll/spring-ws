...
public class DefaultWebServiceTemplateConvention implements WebServiceTemplateConvention {
	...
	private static final KeyValue EXCEPTION_NONE = KeyValue
		.of(WebServiceTemplateObservationDocumentation.LowCardinalityKeyNames.EXCEPTION, KeyValue.NONE_VALUE);
	...
	public KeyValues getLowCardinalityKeyValues(WebServiceTemplateObservationContext context) {
		return KeyValues.of(exception(context), host(context), localname(context), namespace(context), outcome(context),
				soapAction(context));
	}

	private KeyValue path(WebServiceTemplateObservationContext context) {

		return WebServiceTemplateObservationDocumentation.HighCardinalityKeyNames.PATH.withValue(context.getPath());
	}

	private KeyValue localname(WebServiceTemplateObservationContext context) {
		return LowCardinalityKeyNames.LOCALPART.withValue(context.getLocalPart());
	}

	private KeyValue namespace(WebServiceTemplateObservationContext context) {
		return LowCardinalityKeyNames.NAMESPACE.withValue(context.getNamespace());
	}

	private KeyValue host(WebServiceTemplateObservationContext context) {
		return LowCardinalityKeyNames.HOST.withValue(context.getHost());
	}

	private KeyValue outcome(WebServiceTemplateObservationContext context) {
		return LowCardinalityKeyNames.OUTCOME.withValue(context.getOutcome());
	}

	private KeyValue soapAction(WebServiceTemplateObservationContext context) {
		return LowCardinalityKeyNames.SOAPACTION.withValue(context.getSoapAction());
	}

	private KeyValue exception(WebServiceTemplateObservationContext context) {
		if (context.getError() != null) {
			return LowCardinalityKeyNames.EXCEPTION.withValue(context.getError().getClass().getSimpleName());
		}
