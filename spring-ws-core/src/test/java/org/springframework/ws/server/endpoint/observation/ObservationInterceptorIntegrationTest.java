...
public class ObservationInterceptorIntegrationTest {
	...
	void setUp() throws TransformerConfigurationException {
		...
		this.webServiceTemplate = applicationContext.getBean(WebServiceTemplate.class);
		this.registry = applicationContext.getBean(TestObservationRegistry.class);
	...
	void testObservationInterceptorBehavior() {
		...
		MyEndpoint.MyResponse response = (MyEndpoint.MyResponse) this.webServiceTemplate.marshalSendAndReceive(baseUrl,
				request);

		// Assertions based on expected behavior of ObservationInterceptor
		assertNotNull(response);

		assertThat(this.registry).hasAnObservation(
	...
	void testPathWithVariable() {
		...
		MyEndpoint.MyRequest request = new MyEndpoint.MyRequest();
		request.setName("John");
		MyEndpoint.MyResponse response = (MyEndpoint.MyResponse) this.webServiceTemplate
			.marshalSendAndReceive(baseUrl + "/1234", request);
