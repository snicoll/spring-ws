...
class VersionParserTests {
	...
	@Test
	void generationFromRegularVersion() {
		assertThat(Version.from("2.7.14").toGeneration()).isEqualTo("2.7.x");
	}

	@Test
	void generationFromCalVer() {
		assertThat(Version.from("2020.1.0-RC1").toGeneration()).isEqualTo("2020.1.x");
	}

	@Test
	void generationFromVersionProperty() {
		assertThat(Version.from("${spring.version}").toGeneration()).isEqualTo("n/a");
	}
